'''
This code is mainly from https://github.com/lmzintgraf/cavia
'''
import copy
import time
...
class Logger:
    def __init__(self, args, verbose=True):
        ...
        if verbose:
            print(
                ...
                'lr[in]:', args.lr_in, ', [out]:', args.lr_out,
                ', grad[in]:', args.grad_steps_num_train, ', [out]:', args.grad_steps_num_eval,
    ...
    def log_post_update(self, iter_counter, support_x, support_y, query_x, query_y, model, tuned_params=None, mode='train'):

        if iter_counter % self.log_interval == 0:
            if mode == 'train':
                self.training_stats['train_accuracy_post_update'][-1].append(
                    self.get_accuracy(support_x, support_y, model, tuned_params))
                self.training_stats['test_accuracy_post_update'][-1].append(self.get_accuracy(query_x, query_y, model, tuned_params))
                self.training_stats['train_loss_post_update'][-1].append(self.get_loss(support_x, support_y, model, tuned_params))
                self.training_stats['test_loss_post_update'][-1].append(self.get_loss(query_x, query_y, model, tuned_params))
            elif mode == 'valid':
                self.validation_stats['train_accuracy_post_update'][-1].append(
                    self.get_accuracy(support_x, support_y, model, tuned_params))
                self.validation_stats['test_accuracy_post_update'][-1].append(
                    self.get_accuracy(query_x, query_y, model, tuned_params))
                self.validation_stats['train_loss_post_update'][-1].append(self.get_loss(support_x, support_y, model, tuned_params))
                self.validation_stats['test_loss_post_update'][-1].append(self.get_loss(query_x, query_y, model, tuned_params))
            else:
                raise NotImplementedError()
    ...
    def print(self, iter_counter, grad_inner, grad_meta, mode):
        if self.verbose and (iter_counter % self.log_interval == 0) and (mode=='valid'):
